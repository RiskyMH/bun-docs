...
By default, store entries are materialized once into a [global virtual store](/docs/pm/global-store) at `<cache>/links/` and `node_modules/.bun/<pkg>@<ver>` is a symlink into it. Warm installs after `rm -rf node_modules` only create one symlink per package instead of copying every package's files again, which is roughly **7× faster** on a typical mid-size project. See the [global store docs](/docs/pm/global-store) for the full layout, benchmarks, and tradeoffs.
