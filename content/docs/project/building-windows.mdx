...
<Callout type="info">rustup installs the Rust nightly toolchain pinned in `rust-toolchain.toml` on the first build.</Callout>
...
<Callout type="info">ARM64 builds do not need Cygwin because WebKit is provided as a pre-built binary.</Callout>

From here on out, **use a PowerShell terminal with `.\scripts\vs-shell.ps1` sourced**. Load the script by running it:

```ps1 title=".\scripts\vs-shell.ps1"
```
