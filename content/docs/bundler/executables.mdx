...
<Callout type="info">For more examples and patterns, see the [Build-time constants guide](/guides/runtime/build-time-constants).</Callout>
...
<Callout type="warning">These flags cannot be used when cross-compiling because they depend on Windows APIs.</Callout>
...
```ts title="Bun.Build.CompileTarget"
type CompileTarget =
  | "bun-darwin-x64"
