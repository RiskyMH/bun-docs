...
<Callout type="warning">The `.defer()` function can only be called once per `onLoad` callback.</Callout>
...
<Callout type="info">Bun can call this callback from any thread, so the NAPI module implementation must be thread-safe.</Callout>
