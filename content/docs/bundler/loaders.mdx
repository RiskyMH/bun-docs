...
<Callout type="info">In the bundler, `.node` files are handled with the `file` loader.</Callout>
...
<Callout type="warning">The `sqlite` loader is only supported when the target is `bun`.</Callout>
...
<Callout type="info">The location and file name of the copied file are determined by the value of `naming.asset`.</Callout>
