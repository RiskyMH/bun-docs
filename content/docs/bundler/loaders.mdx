...
<Callout type="info">In the bundler, `.node` files are handled using the file loader.</Callout>
...
<Callout type="warning">This is only supported when the target is `bun`.</Callout>
