...
<Callout type="info">Prefer `import.meta.hot.accept()` without an argument; it usually makes your code easier to understand.</Callout>
...
<Callout type="info">For compatibility with Vite, these events are also available with the `vite:*` prefix instead of `bun:*`.</Callout>
