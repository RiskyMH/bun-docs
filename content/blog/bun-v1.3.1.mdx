...
<blockquote className="twitter-tweet"><p lang="en" dir="ltr">In the next version of Bun<br/><br/>`bun build` with sourcemaps &amp; minification enabled gets 2x faster <a href="https://t.co/bjaU6YK9xV">pic.twitter.com/bjaU6YK9xV</a></p>&mdash; Jarred Sumner (@jarredsumner) <a href="https://twitter.com/jarredsumner/status/1977208059241046307?ref_src=twsrc%5Etfw">October 12, 2025</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charSet="utf-8"></script>
...
### Faster installs when no `peerDependencies` are present
...
### `publicHoistPattern` and `hoistPattern`

Bun now supports selective hoisting when using the isolated linker.

- `publicHoistPattern` in `bunfig.toml` (and `public-hoist-pattern` in `.npmrc`) hoists matching transitive dependencies to the root node_modules so tools like ESLint and TypeScript lib augmentations can be discovered across a workspace.
- `hoistPattern` controls what gets hoisted into node_modules/.bun/node_modules.
...
### `FileHandle.readLines()` in `node:fs/promises`
